...
public:
	...
	set<int> possible;
	Block(int log_sum = 0) : logical_sum(log_sum), physical_sum(0), min(1), max(9), empty_cells(0)
	{
		for (int i = 1; i <= 9; i++)
			possible.insert(i);
	}
...
{
public:
	unique_ptr<Block> d_list, r_list;
	k_cell(int x, int y, int _down, int _right) : cell(x, y)
	{
		if (_down)
			d_list = make_unique<Block>(_down);
		if (_right)
			r_list = make_unique<Block>(_right);
	}
