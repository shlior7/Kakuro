		...
		//	cout << "o down: " << o->val << " x,y: " << o->x << " ," << o->y << endl;
			...
			for (int n = 1; n < 10; n++)
				if (possible(o->k_down, o->k_right, n)) {
					...
					if(o->k_right) o->k_right->r_list->add_to_sum(n);
		...
		//	cout << "o right: " << o->val <<" x,y: "<<o->x<<" ,"<<o->y<<endl;
			...
			for (int n = 1; n < 10; n++)
				if (possible(o->k_down, o->k_right, n)) {
					...
					if (o->k_down) o->k_down->d_list->add_to_sum(n);
...
bool Game::possible(const ptk_cell& k_down,const ptk_cell& k_right,int n){
	...
	if (n > k_down->d_list->max || n < k_down->d_list->min) return false;
	if (n > k_right->r_list->max || n < k_right->r_list->min) return false;
